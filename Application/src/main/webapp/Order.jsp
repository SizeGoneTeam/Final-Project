<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<!DOCTYPE html>
<html lang="en" class="font-marketsans mdl-js">

<head>
  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
  <title>BookStore - Summary</title>
  <link rel="stylesheet" href="./My eBay - Summary_files/summary-88zDzmOw.css">
  <link rel="stylesheet" href="css/sumary.css">
</head>

<body>
  <div class="mainContent  summary" id="mainContent" role="main" currency="USD">
    <div class="grid">
      <div class="grid__group">
        <div class="grid__cell grid__cell--16of16 m-page-top">
          <div class="m-top-nav__top">
            <div class="page-title">
              <h1>My BookStore </h1>
            </div>
          </div>
          <div class="m-top-nav__bottom">
            <div class="m-page-tabs">
              <div class="fake-tabs">
                <ul class="fake-tabs__items">
                  <li class="medium-text m-tab selected m-tab__title fake-tabs__item">
                    <a aria-current="page" href="#">Buy mode</a>
                  </li>
                  <li class="medium-text m-tab m-tab__title fake-tabs__item">
                    <a href="summary-sellmode.html">Sell mode</a>
                  </li>
                  <li class="medium-text m-tab m-tab__title fake-tabs__item">
                    <a href="#">Account</a>
                  </li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div class="grid">
      <div class="grid__group">
        <div class="grid__cell grid__cell--3of16 m-page-left">
          <section class="m-links-accordion">
            <ul>
              <li><a href="summary-buymode.html">Tóm tắt</a></li>
              <li><a href="recently-viewed.html">Đã xem gần đây</a></li>
              <li><a href="bidding.html">Bidding</a></li>
              <li class="selected"><a href="#">Đơn mua</a></li>
              <li><a href="canceled.html">Đơn huỷ</a></li>
              <li><a href="watchlist.html">Theo dõi</a></li>
            </ul>
          </section>
        </div>
        <div class="grid__cell grid__cell--13of16 m-page-middle full-width">
          <div class="m-river summary ">
            <div class="m-river__body">
              <div class="m-container container-1">
                <div class="m-container__header border-header">
                  <div class="title-container">
                    <h2 class="title large-text-2">Đã thanh toán</h2>
                    <div class="sub-title regular-text"></div>
                  </div>
                </div>
                <div class="m-container__body">
                  <div class="m-container__body--items">
                    <div class="m-container-items BuyingActivityDwebModule">
                      <div class="m-item-card">
                        <div class="m-container-item-layout-row m-container-item-layout-row__body">
                          <div class="container-item-col container-item-col-img">
                            <a href="#">
                              <div class="m-image"><img
                                  src="https://salt.tikicdn.com/cache/750x750/ts/product/99/10/67/aa71f851071fc05bec537bec76dc4234.jpg.webp"
                                  alt="product's image">
                              </div>
                            </a>
                          </div>
                          <div class="container-item-col container-item-col-item-info">
                            <div
                              class="container-item-col__info-item-info-primary container-item-col__info-item-info-title">
                              <div><a href="#" class="nav-link">Nghĩ giàu và làm giàu</a></div>
                            </div>
                            <div
                              class="container-item-col__info-item-info-primary container-item-col__info-item-info-maxBid">
                              <div>Your max bid: US $35.00</div>
                            </div>
                          </div>
                          <div class="container-item-col container-item-col-orderTotal">
                            <div class="container-item-col__info-tertiary container-item-col__info-displayPrice">
                              <div><span class="POSITIVE BOLD">874,750.00 VND*</span></div>
                            </div>
                            <div class="container-item-col__info-tertiary container-item-col__info-logisticsCost">
                              <div><span class="m-text"><span>+1,010,500.00 VND*</span></span><span
                                  class="m-text">Shipping</span></div>
                            </div>
                          </div>
                          <div class="container-item-col container-item-col-cta">
                            <div class="container-item-col-cta__item">
                              <div class="m-cta">
                                <a href="#" class="default primary fake-btn fake-btn--fluid fake-btn--primary">
                                  Ghé thăm shop
                                </a>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>

              <div class="m-container container-2" id="container-2-anchor" type="MyebayWatchlistModule"
                name="ITEM_CONTAINER" container-id="container-2">
                <div class="title-container">
                  <h2 class="title large-text-2">Chưa thanh toán</h2>
                </div>
                <div class="container-actions border-header bottom-padding"></div>
                <div class="m-container__body">
                  <div class="m-container__body--items">
                    <div class="m-container-items">
                      <div class="m-item-card MyebayActivityItemCard">
                        <div class="m-container-item-layout-row m-container-item-layout-row__body">
                          <div class="container-item-col container-item-col-img">
                            <a href="#">
                              <div class="m-image"><img
                                  src="https://salt.tikicdn.com/cache/750x750/ts/product/99/10/67/aa71f851071fc05bec537bec76dc4234.jpg.webp"
                                  alt="product's image">
                              </div>
                            </a>
                          </div>
                          <div class="container-item-col container-item-col-item-info">
                            <div
                              class="container-item-col__info-item-info-primary container-item-col__info-item-info-title">
                              <div><a href="#" class="nav-link">Nghĩ giàu và làm giàu</a></div>
                            </div>
                            <div
                              class="container-item-col__info-item-info-primary container-item-col__info-item-info-maxBid">
                              <div>Your max bid: US $35.00</div>
                            </div>
                          </div>
                          <div class="container-item-col container-item-col-time-left-info">
                            <div class="container-item-col__info-secondary container-item-col__info-timer">
                              <p>
                                <span class="timer-item">
                                  <span class="NEGATIVE BOLD">7h 45m left</span>
                                </span>
                                <span class="timer-item">
                                  <span class="DEFAULT">Today,</span>
                                </span>
                                <span class="timer-item">
                                  <span class="DEFAULT">01:47 PM PDT</span>
                                </span>
                              </p>
                            </div>
                          </div>
                          <div class="container-item-col container-item-col-orderTotal">
                            <div class="container-item-col__info-tertiary container-item-col__info-displayPrice">
                              <div><span class="POSITIVE BOLD">874,750.00 VND*</span></div>
                            </div>
                            <div class="container-item-col__info-tertiary container-item-col__info-bidCount">
                              <div><a href="#">1 bid</a></div>
                            </div>
                            <div class="container-item-col__info-tertiary container-item-col__info-logisticsCost">
                              <div><span class="m-text"><span>+1,010,500.00 VND*</span></span><span
                                  class="m-text">Shipping</span></div>
                            </div>
                          </div>
                          <div class="container-item-col container-item-col-cta">
                            <div class="container-item-col-cta__item">
                              <div class="m-cta">
                                <a href="#" class="default primary fake-btn fake-btn--fluid fake-btn--primary">
                                  Thanh toán
                                </a>
                              </div>
                            </div>

                            <div class="container-item-col-cta__item">
                              <button class="fake-menu-button__button btn btn--secondary" type="button">
                                <span>Cancel</span>
                              </button>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</body>

</html>